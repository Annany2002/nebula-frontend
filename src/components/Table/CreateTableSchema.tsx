...
export default function CreateTableSchema({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(trimmedTableName)) {
      toast.error("Table name must contain only letters, numbers, and underscores (cannot start with a number)");
      return;
    }
    ...
    const hasReservedId = validColumns.some(
      (col) => col.name.trim().toLowerCase() === "id"
    );
    if (hasReservedId) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-purple-500/5 dark:bg-purple-500/10 border border-purple-200/40 dark:border-purple-500/20 text-xs text-purple-700 dark:text-purple-300">
            ...
            <div className="flex items-center gap-1.5 font-mono text-[11px]">
              <span className="px-1.5 py-0.5 rounded-md bg-purple-500/10 dark:bg-white/10 font-medium">id (INTEGER PK)</span>
              <span className="text-gray-400 dark:text-zinc-500">•</span>
              <span className="px-1.5 py-0.5 rounded-md bg-purple-500/10 dark:bg-white/10 font-medium">created_at (TIMESTAMP)</span>
            </div>
          </div>
